...
public class RdfSchemaShaclJenaValidator implements ConfigurableValidator {
    ...
    private Report createReport(ReportEntry entry, Report.Type type) {
        ...
        String pathStr = entry.resultPath().toString(new Prologue());
        String value = entry.value().toString();
        // Node at focusNode <https://linked.cuzk.cz/resource/ruian/stat/1>
        return reportFactory.message(
                type,
                "NodeShape constraint failed for <"
                        + focus + "> " + pathStr + " " + value);
    }
